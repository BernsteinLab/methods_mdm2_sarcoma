...
if (file.exists(paste0(sample.name,'/promoter_totalLoops_mdm2Bound.tmp'))){
  ...
  #Promoter
  mdm2.counts.matrix <- read.csv('/seq/epiprod02/sgaldon/liposarcoma/chip_master_datasets/bed_files/quant_DD31_mdm2.tmp',sep='\t')
...
if (file.exists(paste0(sample.name,'/promoter_totalLoops_cjunBound.tmp'))){
  ...
  #Promoter
  cjun.counts.matrix <- read.csv('/seq/epiprod02/sgaldon/liposarcoma/chip_master_datasets/bed_files/quant_DD31_cjun.tmp',sep='\t')
  ...
  for (region in cjun.binding.regions){
    ...
    count.1 <- c(count.1,mean(cjun.region[,5]+1, na.rm = T)/mean(cjun.region[,6]+1, na.rm = T))
  }
  
  cjun.counts.lvl <- cbind(cjun.binding.regions,count.1)
  ...
  cjun.enhancers.lvl <- cbind(cjun.binding.regions,count.1)
...
if (file.exists(paste0(sample.name,'/promoter_totalLoops_runxBound.tmp'))){
  ...
  #Promoter
  runx.counts.matrix <- read.csv('/seq/epiprod02/sgaldon/liposarcoma/chip_master_datasets/bed_files/quant_DD31_runx.tmp',sep='\t')
...
table_loop_info <- cbind(paste(total.loops_paste[,1],total.loops_paste[,2],sep='_'),total.loops_paste[,3],genes.list,
                         ...
                         cjun.promoters.list,cjun.enhancers.list,runx.promoters.list,runx.enhancers.list,mdm2.promoters.count1,
...
colnames(table_loop_info) <- c('Loop','Score','Gene Name','Promoter','Enhancer','MDM2 Promoter','MDM2 Enhancer',
                               'cJun Promoter', 'cJun Enhancer','RUNX Promoter', 'RUNX Enhancer', '210222_MDM2_Promoter',
                               '210222_MDM2_Enhancer','210222_cJun_Promoter','210222_cJun_Enhancer',
                               '210222_runx_Promoter','210222_runx_Enhancer')
...
table_loop_info <- cbind(paste(total.loops_paste[,1],total.loops_paste[,2],sep='_'),total.loops_paste[,3],genes.list1, genes.list2,
                         ...
                         runx.promoters1.count1,runx.promoters2.count1)

colnames(table_loop_info) <- c('Loop','Score','Gene.Name.1', 'Gene.Name.2',
                               'Promoter1','Promoter2','MDM2.Promoter1','MDM2.Promoter2',
                               'cJun.Promoter1','cJun.Promoter2','runx.Promoter1', 'runx.Promoter2', '210222_MDM2_Promoter1', '210222_MDM2_Promoter2',
                               '210222_cJun_Promoter1','210222_cJun_Promoter2', '210222_runx_Promoter1','210222_runx_Promoter2')
...
table_loop_info <- cbind(paste(total.loops_paste[,1],total.loops_paste[,2],sep='_'),total.loops_paste[,3],
                         ...
                         mdm2.enhancers2.count1,cjun.enhancers1.count1,cjun.enhancers2.count1,runx.enhancers1.count1,runx.enhancers2.count1)

colnames(table_loop_info) <- c('Loop','Score','enhancer1','enhancer2','MDM2.enhancer1','MDM2.enhancer2',
                               'cJun.enhancer1','cJun.enhancer2','runx.enhancer1', 'runx.enhancer2', '210222_MDM2_Enhancer1',
                               '210222_MDM2_Enhancer2','210222_cJun_Enhancer1','210222_cJun_Enhancer2', '210222_runx_Enhancer1','210222_runx_Enhancer2')


write.table(table_loop_info,file = paste0(sample.name,'/EE_loops/EE_loop_annotation_file.tsv'), sep='\t',quote = F,row.names = F,col.names = T)
...
descriptive.file = matrix(ncol=8)
colnames(descriptive.file) <- c('Promoter','Gene','MDM2','cJun','RUNX', 'MDM2.quant.1', 'cJun.quant.1', 'RUNX.quant.1')
...
for (loop in c(1:dim(a)[1])){
  ...
  if (!a[loop,'Promoter1']%in%descriptive.file[,'Promoter']){
    descriptive.file <- rbind(descriptive.file, c(a[loop,'Promoter1'], a[loop,'Gene.Name.1'], a[loop,'MDM2.Promoter1'],
                                                  a[loop,'cJun.Promoter1'],a[loop,'runx.Promoter1'],a[loop,'X210222_MDM2_Promoter1'], 
                                                  a[loop,'X210222_cJun_Promoter1'],a[loop,'X210222_runx_Promoter1']))
  }
  if (!a[loop,'Promoter2']%in%descriptive.file[,'Promoter']){
    descriptive.file <- rbind(descriptive.file, c(a[loop,'Promoter2'], a[loop,'Gene.Name.2'], a[loop,'MDM2.Promoter2'],
                                                  a[loop,'cJun.Promoter2'],a[loop,'runx.Promoter2'],a[loop,'X210222_MDM2_Promoter2'], 
                                                  a[loop,'X210222_cJun_Promoter2'],a[loop,'X210222_runx_Promoter2']))
  }
}
